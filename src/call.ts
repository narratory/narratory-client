...
const parseDialogflowResponse = async (results, oldContexts, sessionId) => {
  const message : any = results.fulfillmentMessages[0].payload ? struct.decode(results.fulfillmentMessages[0].payload) : { responses: [{
...
const getSessionClient = (agent: Agent) => {
    if (!sessionClient) {
        sessionClient = new dialogflow.SessionsClient(agent.credentials)
    } 
