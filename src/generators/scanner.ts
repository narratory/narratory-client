...
const getAllEntities = (searchable: any, entityIds: string[]): string[] => {
...
async function run() {
  ...
  const files = (await listDir("src")).filter((fileEnt) => fileEnt.isFile()).map((fileEnt) => fileEnt.name)

  for (const file of files) {
    if (file.includes(".ts") && file != "user.ts") {
      try {
        let imports = require(`${process.cwd()}/out/src/${file.replace(".ts", ".js")}`)
        entityIdentifiers = entityIdentifiers.concat(getAllEntities(imports, entityIdentifiers))
      } catch (err) {
  ...
  if (
    !existingFile ||
    sortedUniqueIdentifiers.join() !=
      Object.keys(existingFile)
        .filter((key) => !(key in staticIdentifiers))
        .sort()
        .join()
  ) {
    const allIdentifiers = sortedUniqueIdentifiers
      .concat(staticIdentifiers)
      .map((id) => `${id}: "_${id}",`)
      .join("\n")
